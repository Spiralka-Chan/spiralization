...
public class Patcher {
    ...
    public boolean patchArchive(final Patch patch, final ProgressListener progress) throws IOException {
        final URI uri = URI.create("jar:file:" + new File(gameRootPath, CONFIG_TAR_PATH).getAbsolutePath());
        try (final FileSystem zipfs = FileSystems.newFileSystem(uri, ENV)) {
            ...
            for (int i = 0; i < files.size(); i++) {
                patchFile(files.get(i), patch);
                progress.update(i / (float) (files.size() - 1));
            }
        }
        ...
        for (final String filePath : patch.getCommonFiles()) {
            ...
            if (file != null) {
                try (FileWriter writer = new FileWriter(new File(gameRootPath, filePath))) {
